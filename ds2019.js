 ...
 * programme number are found by PATTERN instead: they are formatted
 * identifiers (N + digits, P-n-nnnnn) that nothing else on the page looks
 * like, and in the sample the SEVIS ID arrives with no label at all.
 *
 * NOT YET RUN AGAINST A REAL DS-2019 PDF. The labels below come from a
 * blank sample of the form itself, so they are the government's own and
 * stable - but pdftext.js could not extract a single character from the
 * CEAC print-out, and nothing here proves it can read a DS-2019 either.
...
(function (root) {
  ...
  /* Formatted identifiers. A SEVIS id is N plus digits and a programme
     number is P-n-nnnnn; nothing else on a DS-2019 is shaped like either.
